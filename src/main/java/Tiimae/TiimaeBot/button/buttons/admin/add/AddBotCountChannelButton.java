...
public class AddBotCountChannelButton implements IButton {
    @Override
    public void handle(ButtonContext event) throws ParseException {
        ...
        settingButtonController.createGuildSettings(event.getEvent().getGuild().getId(), SettingKeyEnum.BOTCOUNTCHANNEL.name(), s[s.length - 1]);
    ...
    @Override
    public String getId() {
        return ButtonEnum.ADDBOTCOUNTCHANNELBUTTON.name();
    }
}
