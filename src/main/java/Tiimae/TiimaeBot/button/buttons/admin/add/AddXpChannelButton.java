...
public class AddXpChannelButton implements IButton {
    ...
    public void handle(ButtonContext event) throws ParseException {
        ...
        settingButtonController.createGuildSettings(event.getEvent().getGuild().getId(), SettingKeyEnum.XPCHANNEL.name(), s[s.length - 1]);
        final long channelId = Long.parseLong(s[s.length - 1]);

        event.getEvent().getMessage().delete().queue();

        channel.sendMessage(String.format(
                "Xp channel has been created on channel %s",
